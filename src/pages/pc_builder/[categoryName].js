...
const CategoryProducts = ({ product, category }) => {
  return (
    <div>
    <Head>
        <title>{category} Choice Page</title>
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <link rel="icon" href="/favicon.ico" />
      </Head>

      <h1 className=" text-5xl text-center font-semibold mt-12">Choose your own {category}</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 p-10 w-full sm:w-3/4 md:w-[80%] mx-auto">
        {product?.map((item, index) => (
          <Product key={index} product={item} isDetailsButton = {false}/>
        ))}
      </div>
    </div>
  );
